"""
Claude Agent SDK style with TraceRoot observability.

Agent built with Anthropic's tool_use API using a clean agent abstraction
(Claude Agent SDK style). TraceRoot auto-instruments all Anthropic LLM calls,
and @observe decorators capture the agent structure.

Note: This example uses the `anthropic` SDK directly with a Claude Agent SDK
style abstraction. It will be updated when the official Claude Agent SDK
package is released.
...
# ---------------------------------------------------------------------------
# Claude Agent SDK style abstraction
# ---------------------------------------------------------------------------


class ClaudeAgent:
    """
    Claude Agent SDK style agent using Anthropic's tool_use API.

    Provides a clean agent abstraction with an explicit tool loop,
    mirroring the pattern of the Claude Agent SDK. TraceRoot
    auto-instruments all LLM calls via Integration.ANTHROPIC; @observe
    decorators capture the agent and tool structure.
    """
